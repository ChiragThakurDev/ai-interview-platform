...
import {
  ...
  History, User, Settings, MessageSquare, X, Zap, Shield, Key,
} from 'lucide-react'
...
const NavItem = ({
  ...
  <NavLink
    ...
    className={({ isActive }) => cn(
      'relative flex items-center gap-2.5 px-3 py-2 rounded text-xs font-medium transition-all duration-150',
      isActive
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        {NAV.map(item => (
          <NavItem key={item.to} {...item} onClick={handleNav} />
        ))}
        ...
        {isAdmin && (
          <div className="pt-3">
            ...
            >
              {({ isActive }) => (
                <>
                  <Shield size={15} className={cn('shrink-0', isActive ? 'text-red-400' : 'opacity-70')} />
